...
kapton = read.csv(text = "power,time,gas,pressure,ratio
...
dmy = dummyVars(" ~ .", data = kapton)
kapton = data.frame(predict(dmy, newdata = kapton))
kapton$power = as.integer(kapton$power)
kapton$time = as.integer(kapton$time)
kapton$gas.Air = as.integer(kapton$gas.Air)
kapton$gas.Argon = as.integer(kapton$gas.Argon)
kapton$gas.Nitrogen = as.integer(kapton$gas.Nitrogen)
kapton$pressure = as.integer(kapton$pressure)


write.csv(kapton, file = "tests/testthat/data/kaptonOnehot.csv")
# clean and save Synthesis Data
library(readr)
MLdata_interface <- read_csv("tests/testthat/data/MLdata_interface.csv")

colnames(MLdata_interface) = c("f", "k", "du", "dv","0tab","0tab2","interface")
synthesis = as.data.frame(MLdata_interface)
synthesis = synthesis[,-c(5,6)]
write.csv(synthesis, file = "tests/testthat/data/synthesis.csv")




# clean and save graphene oxide data
tuning_data_GO_GD_1 <- readxl::read_excel("tests/testthat/data/tuning_data_GO_GD_1.xlsx")
tuning_data_GO_GD_2 <- readxl::read_excel("tests/testthat/data/tuning_data_GO_GD_2.xlsx")
tuning_data_GO_GD_3 <- readxl::read_excel("tests/testthat/data/tuning_data_GO_GD_3.xlsx")

graphene = rbind(tuning_data_GO_GD_1, tuning_data_GO_GD_2, tuning_data_GO_GD_3)

graphene = as.data.frame(graphene)

dmy = dummyVars(" ~ .", data = graphene)
graphene = data.frame(predict(dmy, newdata = graphene))


graphene$power = as.integer(graphene$power)
graphene$time = as.integer(graphene$time)
graphene$gasAir = as.integer(graphene$gasAir)
graphene$gasArgon = as.integer(graphene$gasArgon)
graphene$gasNitrogen = as.integer(graphene$gasNitrogen)
graphene$pressure = as.integer(graphene$pressure)


write.csv(graphene, file = "tests/testthat/data/graphene.csv")


synthesis <- readr::read_csv("tests/testthat/data/synthesis.csv")
synthesis = synthesis[,-1]

openxlsx::write.xlsx(synthesis, "synthesis.xlsx")

graphene <- readr::read_csv("tests/testthat/data/graphene.csv")
graphene = graphene[,-1]

openxlsx::write.xlsx(graphene, "graphene.xlsx")

grapheneOnehot <- readr::read_csv("tests/testthat/data/grapheneOnehot.csv")
grapheneOnehot = grapheneOnehot[,-1]

openxlsx::write.xlsx(grapheneOnehot, "grapheneOnehot.xlsx")
...
kapton_argon <- readr::read_csv("tests/testthat/data/kapton_argon.csv")
kapton_argon = kapton_argon[,-1]

openxlsx::write.xlsx(kapton_argon, "kapton_argon.xlsx")

kaptonOnehot <- readr::read_csv("tests/testthat/data/kaptonOnehot.csv")
kaptonOnehot = kaptonOnehot[,-1]

openxlsx::write.xlsx(kaptonOnehot, "kaptonOnehot.xlsx")
